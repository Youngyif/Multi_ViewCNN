...
def getone(realpath, indexs, region, flag):
    ...
    if flag == -1000 or flag==0:
        ...
        for path, region, i in pathlist:
            ...
            res["imagelist_half1"] = all_image_path[2:18]
    ...
    #########################overlap method#####################################
        ...
        #####16 images
            ...
            if 18-step>20:
                ...
                index2 = (int(indexs)+1)%6
                # print("18-step", region, "indexs", indexs,"index2",index2)
                if int(index2)<int(indexs):
                    region1 = dictlr[region]
                else:
                    region1=region
                pathlist1 = [(realpath + str(indexs), region, 1)]
                pathlist2 = [(realpath + str(index2), region1, 2)]
                for path, region, i in pathlist1:
                    vertical_str = "/vertical_" + region[0] + ".npy"
                    vertical = int(np.load(path + vertical_str))
                    all_image_path = glob.glob(os.path.join(path, "*.jpg"))
                    all_image_path = sorted(all_image_path)
                    ...
                    res["vertical_half1"] = vertical
                    res["region_half1"] = region
                for path, region, i in pathlist2:
                    vertical_str = "/vertical_" + region[0] + ".npy"
                    vertical = int(np.load(path + vertical_str))
                    all_image_path = glob.glob(os.path.join(path, "*.jpg"))
                    all_image_path = sorted(all_image_path)
                ...
                index2 = (int(indexs) - 1) % 6
                # print("2-step", region, "indexs", indexs, "index2", index2)
                if int(index2) > int(indexs):
                    region1 = dictlr[region]
                else:
                    region1=region
                pathlist1 = [(realpath + str(index2), region1, 2)]
                pathlist2 = [(realpath + str(indexs), region, 1)]
                for path, region, i in pathlist1:
                    vertical_str = "/vertical_" + region[0] + ".npy"
                    vertical = int(np.load(path + vertical_str))
                    all_image_path = glob.glob(os.path.join(path, "*.jpg"))
                ...
                # top = (18 - step)%21
                # buttom = (2 - step)%21
                path = realpath + str(indexs)
                vertical_str = "/vertical_" + region[0] + ".npy"
                vertical = int(np.load(path + vertical_str))
                all_image_path = glob.glob(os.path.join(path, "*.jpg"))
                all_image_path = sorted(all_image_path)
                res["imagelist_half1"] = all_image_path[buttom:top]
                res["vertical_half1"] = vertical
                res["region_half1"] = region
